...
class PolynomialFunction:
    ...
    def integration_riser(self,order : int = 1) -> 'PolynomialFunction':
        ...
        for coefficient_index in range(len(self.input_endpoints)-1):
            ...
            riser_coefficient[0] = running_integral-np.sum(interval_left**np.arange(1,_k+2)/np.arange(1,_k+2)*np.array(self.coefficients[coefficient_index]))  
            riser_coefficient[1:] = np.array(self.coefficients[coefficient_index])/np.arange(1,_k+2)
            if interval_right<np.inf:
                ...
                np.sum(interval_left**np.arange(1,_k+2)/np.arange(1,_k+2)*np.array(self.coefficients[coefficient_index]))
...
#               np.max(np.abs(np.array(self.coefficients)-np.array(other.coefficients)) ))
        return np.all(np.abs(np.array(self.input_endpoints[:-1])-np.array(other.input_endpoints[:-1]))<1e-12) and \
               np.all(np.abs(np.array(self.coefficients)-np.array(other.coefficients))<1e-12)
    ...
    for interval_index in range(len(input_endpoints)-1):
        ...
        if G_Es and G_Rs:
            k = max([len(H_each_interval),
                     max([len(G_E_each_interval) for G_E_each_interval in G_Es_each_interval]),
            ...
            k = max([len(H_each_interval),
                     max([len(G_R_each_interval) for G_R_each_interval in G_Rs_each_interval])
                    ])-1
            
        if G_Es:          
            G_Es_each_interval_np = np.zeros(shape=(len(G_Es),k+1))
            for G_E_iter in range(len(G_Es)):
                G_Es_each_interval_np[G_E_iter,:len(G_Es_each_interval[G_E_iter])] = G_Es_each_interval[G_E_iter]
        if G_Rs:                    
            G_Rs_each_interval_np = np.zeros(shape=(len(G_Rs),k+1))
            for G_R_iter in range(len(G_Rs)):
                G_Rs_each_interval_np[G_R_iter,:len(G_Rs_each_interval[G_R_iter])] = G_Rs_each_interval[G_R_iter]            
        ...
        for k_iteration in range(k+1):
            ...
            if G_Es:                      
                y[k_iteration] =  mf.Expr.add(y[k_iteration], 
                                                    mf.Expr.sum(mf.Expr.mulElm(G_Es_each_interval_np[:,k_iteration].tolist(),constraint_ellipsoid_coefficient))
                                                   )
            if G_Rs:
                y[k_iteration] =  mf.Expr.add(y[k_iteration],
                                                    mf.Expr.sum(mf.Expr.mulElm(G_Rs_each_interval_np[:,k_iteration].tolist(),constraint_rectangle_coefficient))
                                                   )
...
def infinite_constraint_basis(M: mf.Model, y: List[mf.Expression], b: float, c: float = np.inf) -> None:
    ...
    k = len(y)-1
    if c==np.inf:
        V = M.variable('PSDmatrix_{:}_{:}'.format(b,c), mf.Domain.inPSDCone(k+1))
        for l in range(1,k+1):
            M.constraint('V_sumIndex=2*{:}-1_interval_left={:}'.format(l,b),mf.Expr.dot(off_diag_auxmat(k,2*l-1),V),mf.Domain.equalsTo(0))
        for l in range(0,k+1):
            RHS_expr = mf.Expr.sum(mf.Expr.vstack([mf.Expr.mul(comb(r,l)*b**(r-l),y[r]) for r in range(l,k+1)]))
            M.constraint('V_sumIndex=2*{:}_interval_left={:}'.format(l,b), mf.Expr.sub(mf.Expr.dot(off_diag_auxmat(k,2*l),V),RHS_expr),mf.Domain.equalsTo(0))
    else:
        W = M.variable('PSDmatrix_{:}_{:}'.format(b,c), mf.Domain.inPSDCone(k+1))
        for l in range(1,k+1):
            M.constraint('W_sumIndex=2*{:}-1_interval_left={:}_interval_right={:}'.format(l,b,c),mf.Expr.dot(off_diag_auxmat(k,2*l-1),W),mf.Domain.equalsTo(0))
        for l in range(0,k+1):
            RHS_expr = mf.Expr.sum(mf.Expr.vstack([ mf.Expr.mul(comb(r,m)*comb(k-r,l-m)*b**(r-m)*c**m,y[r]) for m in range(0,l+1) for r in range(m,k+m-l+1)]))
            M.constraint('W_sumIndex=2*{:}_interval_left={:}_interval_right={:}'.format(l,b,c), mf.Expr.sub(mf.Expr.dot(off_diag_auxmat(k,2*l),W),RHS_expr),mf.Domain.equalsTo(0))    
    return
...
def optimization(D_riser_number: int = None , eta: float = None, eta_lb: float = None, eta_ub: float = None, nu: float = None,
    ...
    if D_riser_number == 2:
        assert (eta_ub is not None) and (eta_lb is not None) and (nu is not None)
    if g_Es is None:
        g_Es = []
    if g_Rs is None:
        g_Rs = []
    ...
    if len(g_Rs)==0 and D_riser_number != 2:
        ...
        G_Rs = [PolynomialFunction([threshold_level, np.inf], [[-threshold_level * nu, nu]])]
        ...
        lambda1 = M.variable("lambda1",mf.Domain.greaterThan(0,mu_ub.getSize()))
        lambda2 = M.variable("lambda2",mf.Domain.greaterThan(0,mu_lb.getSize()))
        rectangle_obj_expr = mf.Expr.sub(mf.Expr.dot(lambda1,mu_ub),mf.Expr.dot(lambda2,mu_lb))
    else:
        if D_riser_number == 0:
            G_Rs = g_Rs
        elif D_riser_number == 1:
        ...
        if D_riser_number == 2:
            G_Rs += [PolynomialFunction([threshold_level, np.inf], [[-threshold_level * nu, nu]])]
    ## objective function
    M.objective('obj',mf.ObjectiveSense.Minimize, mf.Expr.add( mf.Expr.add(kappa,ellipsoid_obj_expr),rectangle_obj_expr))
    ## infinite constraint
...
def test_PolynomialFunction_integration_riser():
    ...
    assert poly1.integration_riser(4)  == PolynomialFunction([1,np.inf],[[1/24,-1/6,1/4,-1/6,1/24]])    
    for _ in range(10):
        a = np.random.rand(1)[0]              
        polya = PolynomialFunction([a,np.inf],[[1]])
        assert polya.integration_riser() == PolynomialFunction([a,np.inf],[[-a,1]])    
        assert polya.integration_riser(1) == PolynomialFunction([a,np.inf],[[-a,1]])
        assert polya.integration_riser(2) == PolynomialFunction([a,np.inf],[[a**2/2,-a,1/2]])    
        assert polya.integration_riser(3) == PolynomialFunction([a,np.inf],[[-a**3/6,a**2/2,-a/2,1/6]])
        assert polya.integration_riser(4)  == PolynomialFunction([a,np.inf],[[a**4/24,-a**3/6,a**2/4,-a/6,1/24]])    
    #### \mathbb{I}(b\leq x\leq c) -> PolynomialFunction([b,c],[[1],[0]])
    poly2 = PolynomialFunction([1,2,np.inf],[[1],[0]])
    assert poly2.integration_riser(1) == PolynomialFunction([1,2,np.inf],[[-1,1],[1,0]])
    assert poly2.integration_riser(2) == PolynomialFunction([1,2,np.inf],[[1/2,-1,1/2],[-3/2,1,0]])    
    for _ in range(10):
        LR = np.random.rand(2)
        L = np.sort(LR)[0]
        R = np.sort(LR)[1]        
        polyLR = PolynomialFunction([L,R,np.inf],[[1],[0]])
        assert polyLR.integration_riser(1) == PolynomialFunction([L,R,np.inf],[[-L,1],[R-L,0]])
        assert polyLR.integration_riser(2) == PolynomialFunction([L,R,np.inf],[[L**2/2,-L,1/2],[-(R-L)*(R+L)/2,R-L,0]])
    ## power functions
    ### x^i\mathbb{I}(x\geq a),i>=0 
    for i in range(0,10):
        a = np.random.rand(1)[0]      
        polyi = PolynomialFunction([a,np.inf],[[0]*i+[1]])
        assert polyi.integration_riser(1) == PolynomialFunction([a,np.inf],[[-a**(i+1)/(i+1)]+[0]*i+[1/(i+1)]])
        assert polyi.integration_riser(2) == PolynomialFunction([a,np.inf],[[a**(i+2)/(i+2)]+[-a**(i+1)/(i+1)]+[0]*(i)+[1/(i+1)/(i+2)]])
